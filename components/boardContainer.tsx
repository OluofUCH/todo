...
export default function BoardContainer() {
 ...
 const [isEditing, setIsEditing] = useState(false);
  ...
  return (
    <>
    {isOpen && <Modal onClose={handleCancel} columnId={id}  editTask={editingTask} isEditing={isEditing}/>}
    <div className='w-full h-full px-4 pt-[36px] grid lg:grid-cols-3 gap-[24px]'>
        <Boardcolumn columnId="To do" onAdd={() => handleAddTask("todoData")} task={todoTasks} refresh={refreshData} edit={handleEditTask}/>
        <Boardcolumn columnId="Ongoing" onAdd={() => handleAddTask("ongoingData")} task={ongoingTasks} refresh={refreshData} edit={handleEditTask}/>
        <Boardcolumn columnId="Done" onAdd={() => handleAddTask("doneData")} task={doneTasks} refresh={refreshData} edit={handleEditTask}/>
    </div>
    </>
  )
}
