...
export default function Options(props: OptionProps) {
  ...
  useEffect(() => {
    ...
    const markHasDone = ()=>{
      if(props.columnId === "doneData"){ return null}
        console.log("HI", props.columnId, props.id);
        const oldData = JSON.parse(localStorage.getItem(props.columnId) || '[]');
  ...
  return (
    <>
    <div ref={optionsRef} className="flex fixed z-99 bg-white shadow-md justify-center h-auto w-auto p-2">
       ...
       {props.columnId === "todoData" && (
         ...
         {(props.columnId === "todoData" || props.columnId === "ongoingData") && (
