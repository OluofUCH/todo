...
export default function Boardcolumn({columnId, onAdd, task, refresh, edit}: BoardColumnProps) {
  ...
  return (
    <div className='h-full min-h-full p-[24px] flex flex-col gap-[24px] w-full rounded-[24px] bg-[#EEF2F5]'>
        ...
        <Taskcard {...data} edit={(taskId) => edit(data.id, data.columnId)} refresh={refresh}/>
        </div>
        ))}
        {task.length === 0 && (
